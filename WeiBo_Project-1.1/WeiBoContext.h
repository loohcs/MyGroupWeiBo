...
@property (nonatomic, strong)NSObject *retweeted_status;//被转发的原来微博信息字段，当该微博为转发时返回
@property (nonatomic, strong)WeiBoContext *retweetedWeibo;//在微博为转发微博时存在
@property (nonatomic, assign)Boolean isRetweeted;//是否转发微博，TRUE：是，FALSE：否
...
@property (nonatomic, strong)NSObject *pic_urls;//微博配图地址。多图时返回多图链接。无配图时返回“[]”
@property (nonatomic, strong)NSMutableArray *weiboPics;
@property (nonatomic, assign)Boolean isHasPics;//是否已经下载了所有图片，TRUE：是，FALSE：否
