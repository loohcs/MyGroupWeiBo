...
{
    __weak PersonalViewController *personVC = self;
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:userID,@"uid", nil];
    personInfoDown = [[WBHTTP_Request_Block alloc] initWithURlString:USERS_SHOW andArguments:dic];
    [personInfoDown setBlock:^(NSMutableData *datas, float progressNum) {
        [personVC loadUserInfoData:datas];
    }];
...
{
    __weak PersonalViewController *personVC = self;
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:userName,@"domain", nil];
    personInfoDown = [[WBHTTP_Request_Block alloc] initWithURlString:USERS_DOMAIN_SHOW andArguments:dic];
    [personInfoDown setBlock:^(NSMutableData *datas, float progressNum) {
        [personVC loadUserInfoData:datas];
    }];
}
