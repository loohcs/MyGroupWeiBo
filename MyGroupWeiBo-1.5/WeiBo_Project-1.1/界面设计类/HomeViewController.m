...
//    [DeleteImageINBundle getImageAttribute];
    
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-84)];
    _scrollView.backgroundColor = [UIColor blackColor];
    _scrollView.userInteractionEnabled = YES;
    _scrollImageView = [[UIImageView alloc] initWithFrame:_scrollView.frame];
...
#warning mark -- 因为下载图片是通过块的调用来实现的，具有一定的延时性。而且下载的图片都是按照顺序一张张下载的，所以我们在下载图片的同时，我们需要确定下载的本张图片到底是属于那一份微博。（目前，我们在微博中使用的都只是缩略图，并未使用图片数组，即pic_urls下载得到的weiboPics。使用数组时，在将图片贴到自定义的weiboCell上会出现一点点小问题，就是多张图片贴的时候会有时混乱。暂定解决思路：在init方法中就定义好九个imageView，然后通过switch语句，依次将图片贴上）
    ...
    //获得本地沙盒存放图片的地址
    NSString *heagImageDirectory = [HomeViewController getImagePath:@"headImage"];
    ...
    for (int i = 0; i <weiboArray.count; i++) {
        ...
        if (oneWeiboContex.retweeted_status != nil) {
            ...
            if (oneWeiboContex.retweetedWeibo.thumbnail_pic.length != 0) {
                ...
                NSString *retWeiboImagePath = [weiboImageDirectory stringByAppendingPathComponent:retWeiboImageName];
        ...
        NSString *headImageName = [oneWeiboContex.userInfo.userIDstr stringByAppendingFormat:@".jpg"];
        NSString *headImagePath = [heagImageDirectory stringByAppendingPathComponent:headImageName];
        ...
        if (oneWeiboContex.thumbnail_pic.length != 0) {
            ...
            NSString *weiboImagePath = [weiboImageDirectory stringByAppendingPathComponent:weiboImageName];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WeiBoContext *oneWeiboContex = [weiboContextsArr objectAtIndex:indexPath.section];
    if (oneWeiboContex.retweeted_status == nil) {
        UITapGestureRecognizer *tapGRWeiboImage = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapWeiboImage:)];
        tapGRWeiboImage.numberOfTapsRequired = 1;
        cell.weiboImage.tag = 1000 + indexPath.section;
        cell.weiboImage.userInteractionEnabled = YES;
        [cell.weiboImage addGestureRecognizer:tapGRWeiboImage];
    }
    else
    {
        UITapGestureRecognizer *tapGRRetWeiboImage = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapRetWeiboImage:)];
        tapGRRetWeiboImage.numberOfTapsRequired = 1;
        cell.retWeiboImage.tag = 1000 + indexPath.section;
        cell.retWeiboImage.userInteractionEnabled = YES;
        [cell.retWeiboImage addGestureRecognizer:tapGRRetWeiboImage];
    }
    
    
    UITapGestureRecognizer *tapGRHeadImage = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapHeadImage:)];
    tapGRHeadImage.numberOfTapsRequired = 1;
    cell.headImage.tag = 2000 + indexPath.section;
    cell.headImage.userInteractionEnabled = YES;
    [cell.headImage addGestureRecognizer:tapGRHeadImage];
    
    
    cell.richText.delegage = self;
    cell.retWeiboText.delegage = self;
...
- (void)richTextView:(TQRichTextView *)view touchEndRun:(TQRichTextBaseRun *)run
{
    if (run.type == richTextURLRunType)
    {
        ...
        {
            WeiboLinkViewController *link = [[WeiboLinkViewController alloc]init];
            [link creatWebView:[NSURL URLWithString:run.originalText]];
            BaseNavigationController *baseNC = [[BaseNavigationController alloc] initWithRootViewController:link];
            baseNC.backButton.hidden = NO;
            baseNC.titleLabel.text = @"链接";
            [self presentViewController:baseNC animated:YES completion:^{
                
            }];
        }
        if ([run.originalText hasPrefix:@"@"]) {
            NSString *screenName = [run.originalText substringFromIndex:1];
            PersonalViewController *personalVC = [[PersonalViewController alloc] init];
            [personalVC getUerName:screenName];
            BaseNavigationController *baseNC = [[BaseNavigationController alloc] initWithRootViewController:personalVC];
            baseNC.backButton.hidden = NO;
            baseNC.titleLabel.text = screenName;
            [self presentViewController:baseNC animated:YES completion:^{
            }];
        }
    }
...
#pragma mark 
#pragma mark -- 微博图片和头像响应的方法
- (void)tapWeiboImage:(UITapGestureRecognizer *)tapGR
{
    __weak HomeViewController *homeVC = self;
    int index = tapGR.view.tag - 1000;
    WeiBoContext *oneWeiboContex = [weiboContextsArr objectAtIndex:index];
    
    [self presentImageView:oneWeiboContex.thumbnailImage andClass:homeVC];
    [self.view addSubview:self.scrollView];
    
    ImageDownload *imageDown = [[ImageDownload alloc] initWithURlString:oneWeiboContex.original_pic];
    [imageDown setBlock:^(NSMutableData *datas, float progressNum) {
        [homeVC getOriginalImage:datas andClass:homeVC];
    }];
}

- (void)getOriginalImage:(NSMutableData *)data andClass:(HomeViewController *)homeVC
{
...
//    homeVC.scrollView.backgroundColor = [UIColor blackColor];
//    float imageViewHeight = 320*image.size.height/image.size.width;
//    homeVC.scrollView.pagingEnabled = NO;
//    homeVC.scrollView.bounces = NO;
//
//    if (image.size.height > homeVC.scrollView.frame.size.height) {
//        homeVC.scrollView.contentSize = CGSizeMake(320, imageViewHeight);
//        homeVC.scrollImageView.frame = CGRectMake(0, 0, 320, imageViewHeight);
//    }
//    else
//    {
//        homeVC.scrollView.contentSize = CGSizeMake(320, imageViewHeight);
//        homeVC.scrollImageView.frame = CGRectMake(0, (SCREEN_HEIGHT-imageViewHeight - 84)/2, 320, imageViewHeight);
//    }
    
    [self presentImageView:image andClass:homeVC];
    [homeVC.view addSubview:homeVC.scrollView];
    
    
//    UITapGestureRecognizer *tapGR = [[UITapGestureRecognizer alloc] initWithTarget:homeVC action:@selector(hideImageView:)];
    
    homeVC.scrollView.userInteractionEnabled = YES;
    
//    tapGR.numberOfTapsRequired = 2;
//    [homeVC.scrollView addGestureRecognizer:tapGR];
//    [self.view addSubview:_scrollView];
    
}

- (void)presentImageView:(UIImage *)image andClass:(HomeViewController *)homeVC
{
    homeVC.scrollView.backgroundColor = [UIColor blackColor];
    float imageViewHeight = 320*image.size.height/image.size.width;
    homeVC.scrollView.pagingEnabled = NO;
    ...
    homeVC.scrollImageView.image = image;
...
- (void)hideImageView:(UITapGestureRecognizer *)tapGR
{
...
- (void)tapRetWeiboImage:(UITapGestureRecognizer *)tapGR
{
    __weak HomeViewController *homeVC = self;
    int index = tapGR.view.tag - 1000;
    WeiBoContext *oneWeiboContex = [weiboContextsArr objectAtIndex:index];
    ...
    ImageDownload *imageDown = [[ImageDownload alloc] initWithURlString:oneWeiboContex.retweetedWeibo.original_pic];
...
- (void)tapHeadImage:(UITapGestureRecognizer *)tapGR
{
    int index = tapGR.view.tag - 2000;
    WeiBoContext *oneWeiboContex = [weiboContextsArr objectAtIndex:index];
    
    PersonalViewController *personalVC = [[PersonalViewController alloc] init];
    BaseNavigationController *baseNC = [[BaseNavigationController alloc] initWithRootViewController:personalVC];
    baseNC.backButton.hidden = NO;
    baseNC.titleLabel.text = oneWeiboContex.userInfo.name;
    [personalVC getUserInfo:oneWeiboContex.userInfo.userIDstr];
    [self presentViewController:baseNC animated:YES completion:^{
...
- (void)retweetedWeibo:(UIButton *)sender
{
    ...
    WeiBoContext *oneweiboContex = [weiboContextsArr objectAtIndex:sender.tag - 100];
    NSLog(@"---------------%d", oneweiboContex.ID);
    NSLog(@"%@", oneweiboContex.idstr);
...
- (void)commentWeibo:(UIButton *)sender
{
    ...
    WeiBoContext *oneweiboContex = [weiboContextsArr objectAtIndex:sender.tag - 200];
    NSLog(@"%d", oneweiboContex.ID);
    NSLog(@"%@", oneweiboContex.idstr);
