...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    switch (buttonIndex) {
        ...
        case 1:
        {
            if (_style != 0)
            {
                NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithInt:_style],@"type", _weiboContext.userInfo.name, @"writer", [NSString stringWithFormat:@"%d", _weiboContext.ID], @"weiboID", _textView.text,@"text",  nil];
...
- (void)addWeiboContex:(WeiBoContext *)oneWeiboContex andContexStyle:(ContexStyle)aStyle
{
    ...
    NSString *headImagePath = [headImageDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.jpg", oneWeiboContex.userInfo.userIDstr]];
    NSString *weiboImagePath = [weiboImageDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.jpg", oneWeiboContex.idstr]];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    
    if (oneWeiboContex.retweeted_status == nil)
    {
        if ([fileManager fileExistsAtPath:weiboImagePath]) {
            //将微博的图片贴上
    ...
    {
        NSString *retWeiboImagePath = [weiboImageDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.jpg",oneWeiboContex.retweetedWeibo.idstr]];
        if ([fileManager fileExistsAtPath:retWeiboImagePath]) {
            //将微博的图片贴上
            UIImage *image = [UIImage imageWithContentsOfFile:weiboImagePath];
            imageView.image = image;
            [_weiboView addSubview:imageView];
...
- (void)finishAction
{
    ...
    if (_style == 0)
    {
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:statuses, @"status", @"0", @"visible", nil];
    ...
    if (_style == 1) {
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:statuses, @"status", @"0", @"visible", _weiboContext.idstr, @"id", nil];
        [WBHttpRequest requestWithAccessToken:[defaluts objectForKey:@"accessToken"] url:STATUSES_REPOST
                                   httpMethod:@"POST" params:dic delegate:self];
    }
    if (_style == 2) {
        NSDictionary *dic = [[NSDictionary alloc] initWithObjectsAndKeys:statuses, @"comment", _weiboContext.idstr, @"id", nil];
